...
def main() -> int:
    ...
    add("")
    add("/**")
    add(" * Trang bệnh tương ứng với một cây trong danh mục trang chủ, nếu có.")
    add(" *")
    add(" * Trang chủ dùng hàm này để trỏ thẳng từ thẻ cây sang trang bệnh của nó.")
    add(" * Trả null khi cây chưa đủ hai bệnh, vì khi đó trang không được sinh và")
    add(" * liên kết sẽ dẫn tới 404.")
    add(" */")
    add("export function diseasePageForPlant(plantId: string): CropWithDiseases | null {")
    add("  const crop = CROP_DISEASES.find((entry) => entry.plantId === plantId);")
    add("  return crop && crop.diseases.length >= 2 ? crop : null;")
    add("}")
    add("")
