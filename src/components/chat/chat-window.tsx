  ...
  return (
    <Card variant="raised" padding="none" className="overflow-hidden rounded-xl">
      <div className="flex items-center justify-between border-b border-line px-5 py-4">
        <div><p className="text-sm font-bold text-ink">{tr("Cuộc trò chuyện", "Conversation")}</p><p className="mt-0.5 text-xs text-ink-soft">{tr("Mô tả càng cụ thể, câu trả lời càng sát bối cảnh.", "The more specific your description, the more relevant the answer.")}</p></div>
      ...
      <div className="max-h-[580px] min-h-[420px] space-y-4 overflow-y-auto bg-canvas/40 px-4 py-5 sm:px-5">
        {!messages.length ? (
          <div className="flex min-h-[330px] flex-col items-center justify-center px-5 text-center">
            ...
            <p className="mt-4 text-base font-bold text-ink">{tr("Bạn cần hỗ trợ điều gì?", "What do you need help with?")}</p>
            <p className="mt-2 max-w-md text-sm leading-7 text-ink-soft">{tr("Chọn một câu hỏi gợi ý hoặc nhập vấn đề về ảnh lá, dấu hiệu quan sát được và cách theo dõi tiếp.", "Pick a suggested question or describe an issue about the leaf image, the symptoms you observed and how to keep tracking.")}</p>
          </div>
        ) : null}
        ...
        {typing ? (
          <div className="flex justify-start" role="status">
            <div className="inline-flex items-center gap-2 rounded-lg border border-line bg-surface px-4 py-3 text-sm text-ink-soft shadow-sm">
              ...
              {tr("Đang soạn phản hồi...", "Composing a response...")}
            </div>
          </div>
        ) : null}
      </div>
